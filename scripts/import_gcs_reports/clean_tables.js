...
async function clean() {
  ...
  const targetTables = tables
    .map((t) => t.id)
    .filter(
      (name) => name.endsWith('_timeseries') || HISTOGRAM_TABLES.includes(name)
    )

  console.log(
    `Found ${targetTables.length} tables to truncate. Running concurrently (limit 20)...`
  )
  ...
  await mapLimit(targetTables, 20, async (name) => {
    ...
    try {
      await bigquery.query({
        query: `TRUNCATE TABLE \`${datasetId}.${name}\``
      })
    } catch (e) {
